...
$FileNameRun = switch -Wildcard ($FileName) {
    ...
    default {
        $FileName -replace '\.intunewin$', '.ps1' 
    }
}
#endregion
...
$StartupCommand = "powershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$SandboxDesktopPath\bin\${PackageFolderName}_PreLogonCommand.ps1`""
